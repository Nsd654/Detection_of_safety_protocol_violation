...
def post_message(camera_id, output_dict, image, min_score_thresh):
    ...
    for person_box in person_boxes:
        ...
        if not person["hardhat"] and not person["hardhat"] :
    ...
    try:
        ...
        files = {
            'file': ('file', open(IMAGE_FILE, 'rb'), 'multipart/form-data', {'Expires': '0'})
        }
        # files = {'file': open(IMAGE_FILE, 'rb')}
        headers = {'Content-type': 'multipart/form-data'}
        if len(persons):
            result = requests.post(" http://192.168.43.195:5000/img",files=files)
            result = requests.post(" http://192.168.43.195:5000/noti",json=message)
        ...
        with tf.Session() as sess:
            ...
            while True:
                ...
                if time.time() - send_message_time > config.message_send_interval / 1000.0:
                    ...
                    try:
                        print (camera_id,output_dict,config.object_confidence_threshold,"=====================")
...
def main():
    ...
    video_processing(graph, category_index, "demo_video.mp4", 1, 0, run_flag, message_queue)
    p.join()
    
    #image_processing(graph, category_inde     x, './examples/002.jpg', True)
